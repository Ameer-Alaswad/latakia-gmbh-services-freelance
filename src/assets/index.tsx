export const images = [
    {
        label: "security",
        imgPath: "security-service.jpg",
        textOne: "Zuverlässiger Schutz für Ihr Vermögen",
        textTwo: "Umfassende Sicherheitsdienste, die auf Ihre Geschäftsanforderungen zugeschnitten sind",
    },
    {
        label: "cleaning",
        imgPath: "cleaning.jpg",
        textOne: "Makellose Räume, gesünderes Leben",
        textTwo: "Professionelle Reinigung, maßgeschneidert auf Ihre Bedürfnisse zu Hause und im Büro",
    },
    {
        label: "Car rental entrepreneur",
        imgPath: "person-transportation.jpg",
        textOne: "Genießen Sie eine stressfreie Fahrt mit unseren umfassenden Mietwagen-Dienstleistungen.",
        textTwo: "Beginnen Sie Ihre Reise ohne Probleme mit unserer großen Auswahl an Autos und reibungslosen Mietprozess.",
    },
    {
        label: "logistics",
        imgPath: "Logistics-service.jpg",
        textOne: "Effiziente Lösungen für Ihre Logistikanforderungen",
        textTwo: "Wir liefern Ihre Waren jederzeit sicher und pünktlich",
    },
];

export const servicesOverviewData = [
    {
        img: "services-images/cleaning-service.jpg",
        title: "Reinigungsservice",
        description:
            "Wir spezialisieren uns auf umfassende Reinigungsdienstleistungen für Wohnungen, Häuser, Büros und mehr, um sicherzustellen, dass jeder Winkel sauber ist.",
    },
    {
        img: "services-images/logistics-services.jpg",
        title: "Logistik Service",
        description:
            "Wir Transportieren Ihre Ware zum gewünschten Zielort Bundesweit, Erleben Sie die Bequemlichkeit der Tür-zu-Tür-Lieferung mit unserem landesweiten Versandnetz.",
    },
    {
        img: "services-images/security-service.jpg",
        title: "Sicherheitsdienst",
        description:
            "Wir bieten eine breite Palette von Dienstleistungen an, von Objektbewachung und Notunterkünften über Veranstaltungen und Pförtnerdienste bis hin zur Citystreife und Brandwache, um Ihre Sicherheit in verschiedenen Bereichen zu gewährleisten.",
    },
    {
        img: "person-transportation.jpg",
        title: "Mietwagen Unternehmer",
        description:
            "Wir bieten umfassende Mietwagendienste an, von der Vermittlung von Mietwagenunternehmen bis hin zur Unterstützung bei der Fahrzeugversicherung und -finanzierung, um Ihnen eine bequeme und sichere Reise zu ermöglichen.",
    },
];


